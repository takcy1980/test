...
@Component
public class LoginProviderCustomers implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        Iterator<Customer_accounts> accounts = HibernateEntityHelper.all(Customer_accounts.class).
                stream().
                filter(a -> a.getLogin().equals(name)).iterator();
        
        while(accounts.hasNext()){
            Customer_accounts account = accounts.next();
            Boolean check = false;
            try{
